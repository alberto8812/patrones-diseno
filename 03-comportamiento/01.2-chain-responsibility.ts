...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 1000) {
      console.log(`${COLORS.brown}La solicitud de compra de $${amount} ha sido aprobada por el Supervisor.${COLORS.brown}`);
    } else {
      this.next(amount);
    }
  }
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 5000) {
      console.log(`${COLORS.orange}La solicitud de compra de $${amount} ha sido aprobada por el Gerente.${COLORS.orange}`);
      return
    }
    this.next(amount);
  }
...
class Director extends BaseApprover {
  override approveRequest(amount: number): void {
    if (amount > 5000) {
      console.log(`${COLORS.green}La solicitud de compra de $${amount} ha sido aprobada por el Director.${COLORS.green}`);
      return
    }
    this.next(amount);
  }
